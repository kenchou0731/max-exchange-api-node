  ...
  _handleMessage (body) {
    ...
    if (channel === 'user') {
      switch (eventType) {
        case 'order_update':
        case 'order_snapshot':
          this.emit('user.' + eventType.replace('_', '.'), parseUserOrders(obj.o))
          break

        case 'trade_update':
        case 'trade_snapshot':
          this.emit('user.' + eventType.replace('_', '.'), parseUserTrades(obj.t))
          break
...
function parseUserOrders (orders) {
  return orders.map((o) => {
    return {
      id: o.i,
      client_oid: o.ci,
      side: o.sd === 'bid' ? 'buy' : 'sell',
      ord_type: o.ot,
      price: new Decimal(o.p),
      stop_price: o.sp === null ? null : new Decimal(o.sp),
      avg_price: new Decimal(o.ap),
      state: o.S,
      market: o.M,
      time: new Date(o.T),
      volume: new Decimal(o.v),
      remaining_volume: new Decimal(o.rv),
      executed_volume: new Decimal(o.ev),
      trades_count: o.tc,
      group_id: o.gi
    }
  })
...
function parseUserTrades (trades) {
  return trades.map((o) => {
    return {
      id: o.i,
      price: new Decimal(o.p),
